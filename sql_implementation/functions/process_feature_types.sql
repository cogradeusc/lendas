DROP FUNCTION IF EXISTS  process_feature_types(feature_types JSON, schema_name text) CASCADE;

CREATE OR REPLACE FUNCTION process_feature_types(feature_types JSON, schema_name text)
RETURNS void
AS 
$$
DECLARE 
...
BEGIN
 ...
 FOR feature_type IN 
   ...
   FROM json_array_elements(feature_types) ft(f)
 LOOP 
    ...
    -- insert the metadata of the feature type
      INSERT INTO lendas_catalog.feature_types
      VALUES (schema_name,
           feature_type.name, 
