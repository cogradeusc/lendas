 ...
 LOOP 
    ...
    WHERE NOT EXISTS (SELECT *
     ...
     WHERE lower(r->>'scope')='valid_time_period';
      ...
      --check that all vocabularies of keywords exist
    SELECT array_agg(coalesce((lendas_process_qualified_name(lower(k->>'vocabulary'))).schema_name,schema_name)||'.'||
                              (lendas_process_qualified_name(lower(k->>'vocabulary'))).unqualified_name)
    INTO undefined_vocabularies
    FROM json_array_elements(feature_type.keywords) AS keywords(k)
    WHERE k->>'vocabulary' IS NOT NULL
      AND k->>'vocabulary' <> ''
      AND NOT EXISTS (SELECT *
                      FROM lendas_catalog.vocabularies voc
                      WHERE voc.schema = coalesce((lendas_process_qualified_name(lower(k->>'vocabulary'))).schema_name,schema_name)
                        AND voc.name = (lendas_process_qualified_name(lower(k->>'vocabulary'))).unqualified_name);
     
     IF array_length(undefined_vocabularies,1)>0 THEN
        RAISE EXCEPTION 'The following vocabularies have not been defined in keywords section: %',undefined_vocabularies;
     END IF;


    -- obtain the list of keywords with processed vocabularies
     SELECT json_agg(json_build_object('keyword', k->'keyword',
                                       'vocabulary',
                                       CASE 
                                       	 WHEN k->>'vocabulary' IS NULL OR k->>'vocabulary' = '' THEN NULL 
                                       	 ELSE json_build_object('schema',coalesce((lendas_process_qualified_name(lower(k->>'vocabulary'))).schema_name,schema_name),
                                                                'name', (lendas_process_qualified_name(lower(k->>'vocabulary'))).unqualified_name)
                                       END
                                     ) 
                     ) into feature_type_keywords
      FROM json_array_elements(feature_type.keywords) AS keywords(k);
      ...
      generated_foi:= json_build_object('shape_type', lower(feature_type.generated_feature_of_interest_type->>'shape_type'),
       ...
       END IF;
	    ...
	    IF feature_type.generated_feature_of_interest_type IS NOT NULL THEN
	        IF feature_type.result_time_type = 'instant' THEN
               IF feature_type.phenomenon_time_type = 'instant' THEN
                 create_table_query:=create_table_query || ', primary key (procedure, result_time, phenomenon_time)';
               ELSEIF feature_type.phenomenon_time_type = 'period' THEN
                  create_table_query:=create_table_query || ', exclude using GIST (procedure with =, result_time with =, phenomenon_time with &&)';
               ELSE
        ...
        ELSE
            IF feature_type.result_time_type = 'instant' THEN
               IF feature_type.phenomenon_time_type = 'instant' THEN
                 create_table_query:=create_table_query || ', primary key (procedure, feature_of_interest, result_time, phenomenon_time)';
               ELSEIF feature_type.phenomenon_time_type = 'period' THEN
                  create_table_query:=create_table_query || ', exclude using GIST (procedure with =, feature_of_interest with =, result_time with =, phenomenon_time with &&)';
               ELSE
                 create_table_query:=create_table_query || ', primary key (procedure, feature_of_interest, result_time)';
               END IF;
 			ELSE 
               IF feature_type.phenomenon_time_type = 'instant' THEN
                 create_table_query:=create_table_query || ', exclude using GIST (procedure with =, feature_of_interest with =, result_time with &&, phenomenon_time with =)';
               ELSEIF feature_type.phenomenon_time_type = 'period' THEN
                  create_table_query:=create_table_query || ', exclude using GIST (procedure with =, feature_of_interest with =, result_time with &&, phenomenon_time with &&)';
               ELSE
                 create_table_query:=create_table_query || ', exclude using GIST (procedure with =, feature_of_interest with =, result_time with &&)';
               END IF;
			END IF;
	    END IF;
