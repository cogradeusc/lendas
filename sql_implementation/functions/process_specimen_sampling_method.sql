DROP FUNCTION IF EXISTS  process_specimen_sampling_method_type(sampling_method_type JSON, schema_name text) CASCADE;

CREATE OR REPLACE FUNCTION process_specimen_sampling_method_type(sampling_method_type JSON, schema_name text)
RETURNS json
AS 
$$
DECLARE 
...
BEGIN
  ...
  EXECUTE FORMAT('CREATE SCHEMA IF NOT EXISTS %s',schema_name);	
	...
	--process the feature_type
   SELECT lower(sampling_method_type->>'name') AS name,
          sampling_method_type->>'description' AS description,
          sampling_method_type->'names' AS names,
          sampling_method_type->'properties' AS properties,
          sampling_method_type->'references' AS REFERENCES
   INTO feature_type;
...
RETURN json_build_object('name', feature_type.name,
                         'description', feature_type.description,
                          'names', feature_type_names,
                          'properties', feature_type_properties,
                          'references', feature_type_references
                        );
